...
function startNewGame() {
  fetch("https:108.35.7.75:8080/newgame")
...
function join(id) {
  fetch(`https:108.35.7.75:8080/join?id=${id}`).then(() => {
    color = "b";
    turn = false;
...
function endGame(id) {
  fetch("https:108.35.7.75:8080/end", {
    method: "POST",
    body: id,
  });
}
...
function rematch(id) {
  fetch(`https:108.35.7.75:8080/rematch?id=${id}`)
    .then((response) => response.json())
    .then((data) => {
      tilesdata = data;
      color = color === "w" ? "b" : "w";
...
function establishConnection(id) {
  fetch(`https:108.35.7.75:8080/establish?id=${id}`).then(() => {
    board1 = ChessBoard("board1", "start");
    addListeners();
...
function sendMove(move) {
  fetch("https:108.35.7.75:8080/send", {
    method: "POST",
    body: move,
  })
    .then((response) => response.json())
...
function getMove(id) {
  fetch(`https:108.35.7.75:8080/getmove?id=${id}`)
    .then((response) => response.json())
    .then((data) => {
      tilesdata = data.tiles;
